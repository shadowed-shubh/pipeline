...
def gemini_summary(report):
    try:
        ...
        out = client.models.generate_content(
            ...
            contents="Simplify for patient in plain English: " + json.dumps(report),
        )
        return out.text
    except Exception as e:
...
# ----------------------------------------
# Diagnose Route (Gemini + gTTS in parallel)
# ----------------------------------------
@app.post("/diagnose")
async def diagnose(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Step 2 — Groq LLM report
        loop = asyncio.get_event_loop()
        report = await loop.run_in_executor(
            executor, generate_llm_report, prediction, confidence
        )

        # Step 3 — Gemini summary + gTTS voice in parallel
        summary, voice_generated = await asyncio.gather(
            loop.run_in_executor(executor, gemini_summary, report),
            loop.run_in_executor(executor, generate_voice, report)
        )
